...
componentWillUnmount() {
    clearInterval(this.timerIDpl);
  }

tickpl() {
    console.log('Puhelinluettelo tickpl ', this.state.showNoteC)
    this.setState({showNoteC: false})
    clearInterval(this.timerIDpl);
    ...
    in NoteC (at Puhelinluettelo.js:181)
    in div (at Puhelinluettelo.js:180)
    */
   /*
    TODO 5.10.2018
    DELETE http://localhost:3001/persons/17 404 (Not Found)
Puhelinluettelo.js:220 remove error Error: Request failed with status code 404
  ...
  /*
  TODO 5.10.2018
  Poisto toiminnon (remove) jälkeen ohjelma näyttäisi jäävän
  tickpl osalta käyntiin ja looppiin? 
  */
  }
...
// TODO miten välitetään arvo tähän poistoon valitusta rivistä
      ...
      .remove(arvo)
        ...
        this.setState({showNoteC: true})    
        this.timerIDpl = setInterval(
            () => this.tickpl(),
            6000);
